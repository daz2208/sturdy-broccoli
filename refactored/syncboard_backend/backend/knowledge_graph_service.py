...
class KnowledgeGraphService:
    ...
    def build_graph(
        ...
        for doc in documents:
            # Get document-level summary (level 3)
            summary = db.query(DBDocumentSummary).filter(
                DBDocumentSummary.document_id == doc.id,
                DBDocumentSummary.summary_level == 3
            ).first()

            concepts = summary.key_concepts if summary and summary.key_concepts else []
            tech_stack = summary.tech_stack if summary and summary.tech_stack else []
