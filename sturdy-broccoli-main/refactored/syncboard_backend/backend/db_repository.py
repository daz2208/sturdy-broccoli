...
class DatabaseKnowledgeBankRepository(KnowledgeBankRepository):
    ...
    async def delete_cluster(self, cluster_id: int) -> bool:
        ...
        Documents in the cluster will have their cluster_id set to NULL (become unclustered).
        This is handled automatically by the database foreign key constraint (ondelete="SET NULL").
        ...
        async with self._lock:
            ...
            doc_count = len(db_cluster.documents)

            self.db.delete(db_cluster)
            self.db.commit()
            logger.info(f"Deleted cluster {cluster_id} '{cluster_name}' ({doc_count} documents now unclustered)")
            return True
