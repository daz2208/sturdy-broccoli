...
function showUploadType(type) {
    ...
    if (type === 'text') {
        ...
        forms.innerHTML = `
            <textarea id="urlInput" rows="5" placeholder="Enter URLs (one per line, or separated by spaces/commas)&#10;https://youtube.com/watch?v=...&#10;https://example.com/article&#10;https://wikipedia.org/page"></textarea>
            <button onclick="uploadUrls(event)">Upload URLs</button>
            <p style="color: #888; font-size: 0.85rem; margin-top: 5px;">
                Enter one or more URLs. Separate with newlines, spaces, or commas. Supports YouTube, web articles, PDFs. Max 10 URLs per batch.
            </p>
            <div id="urlBatchProgress" style="display: none; margin-top: 10px;">
                <div style="background: #333; border-radius: 4px; overflow: hidden;">
                    <div id="urlBatchProgressBar" style="height: 8px; background: #4CAF50; width: 0%; transition: width 0.3s;"></div>
                </div>
                <p id="urlBatchProgressText" style="color: #888; font-size: 0.85rem; margin-top: 5px;">Processing 0 of 0...</p>
            </div>
        `;
    } else if (type === 'file') {
        forms.innerHTML = `
...
// Batch URL upload handler - uploads multiple URLs in one request
async function uploadUrls(event) {
    const button = event ? event.target : null;
    ...
    // Parse URLs - supports newlines, spaces, commas, or semicolons as separators
    const urls = urlText
    ...
    // For single URL, use the regular upload
    if (urls.length === 1) {
        return uploadSingleUrl(urls[0], button);
    }

    // Limit to 10 URLs
    if (urls.length > 10) {
        showToast('Maximum 10 URLs allowed per batch upload', 'error');
        return;
    }
    ...
    // Show progress bar
    const progressDiv = document.getElementById('urlBatchProgress');
    const progressBar = document.getElementById('urlBatchProgressBar');
    const progressText = document.getElementById('urlBatchProgressText');
    if (progressDiv) {
        progressDiv.style.display = 'block';
        progressBar.style.width = '0%';
        progressText.textContent = `Preparing ${urls.length} URLs...`;
    }

    try {
        progressText.textContent = `Uploading ${urls.length} URLs...`;
        progressBar.style.width = '20%';

        // Send batch upload request
        const res = await fetch(`${API_BASE}/upload_batch_urls`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
        ...
        if (res.ok) {
            ...
            showToast(`📤 Batch queued: ${data.queued} of ${data.total_urls} URLs`, 'info');

            // Show any errors
            if (data.errors && data.errors.length > 0) {
                data.errors.forEach(err => {
                    showToast(`⚠️ ${err.url}: ${err.error}`, 'error');
                });
            }
            ...
            if (data.jobs && data.jobs.length > 0) {
                ...
                if (button) setButtonLoading(button, false, 'Upload URLs');
            }
        } else {
            const errorMsg = await getErrorMessage(res);
            showToast(errorMsg, 'error');
        ...
        showToast('Batch URL upload error: ' + e.message, 'error');
        if (progressDiv) progressDiv.style.display = 'none';
        if (button) setButtonLoading(button, false, 'Upload URLs');
    }
}

// Single URL upload (called by uploadUrls for single URL)
async function uploadSingleUrl(url, button) {
    if (button) setButtonLoading(button, true);
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Poll for job status
            pollJobStatus(data.job_id, button, 'Upload URLs');
        } else {
            const errorMsg = await getErrorMessage(res);
            showToast(errorMsg, 'error');
            if (button) setButtonLoading(button, false, 'Upload URLs');
...
async function searchKnowledge() {
    ...
    if (searchMode === 'summary') {
        ...
        try {
            const response = await performSummarySearch(query, summaryLevel, 20);
            displaySummarySearchResults(response.results);
        } catch (e) {
...
async function searchByConcept(concept = null) {
    ...
    try {
        ...
        const data = await response.json();
        const results = data.documents || [];  // Extract documents array from response
...
async function performSummarySearch(query, level = 'all', limit = 20) {
    // Map string level to integer for backend
    const levelMap = {
        'chunk': 1,
        'section': 2,
        'document': 3
    ...
    const levelValue = level === 'all' ? '' : `&level=${levelMap[level] || level}`;

    const response = await fetch(
        `${API_BASE}/search/summaries?q=${encodeURIComponent(query)}${levelValue}&limit=${limit}`,
        { headers: { 'Authorization': `Bearer ${token}` } }
    );
...
/**
 * Clear search results and hide back button.
 */
                ...
                <div style="display: flex; justify-content: space-between; align-items: start;">
                    <div>
                        ...
                        <span style="background: #a78bfa22; color: #a78bfa; padding: 2px 8px; border-radius: 10px; font-size: 0.75rem; margin-left: 10px;">
                            ${r.summary_type || 'document'}
                        </span>
                    </div>
                    <span style="color: #888; font-size: 0.85rem;">${r.source_type || 'unknown'}</span>
                </div>
                <p style="color: #e0e0e0; margin: 10px 0; font-size: 0.9rem;">${r.short_summary || r.detailed_summary || 'No summary available'}</p>
                ${r.key_concepts && r.key_concepts.length > 0 ? `
                    <div class="concepts-list" style="margin-top: 10px;">
                        ${r.key_concepts.slice(0, 5).map(c => `<span class="concept-tag">${c}</span>`).join('')}
                    </div>
                ` : ''}
            </div>
        `).join('')}
