...
Place this in: backend/routers/knowledge_bases.py
"""
...
async def list_knowledge_bases(
    ...
    return KnowledgeBaseList(
        knowledge_bases=[KnowledgeBase.from_orm(kb) for kb in bases],
        total=len(bases)
    )
...
async def create_knowledge_base(
    ...
    return KnowledgeBase.from_orm(kb)
...
async def generate_build_suggestions(
    ...
    # TODO: Call your existing build suggestion logic
    # This will be integrated with your improved build suggester
    ...
    docs = {
        doc_id: content 
        for doc_id, content in dependencies.documents.items()
        if dependencies.metadata.get(doc_id) and 
           dependencies.metadata[doc_id].knowledge_base_id == kb_id
    }
    
    metadata = {
        doc_id: meta
        for doc_id, meta in dependencies.metadata.items()
        if meta.knowledge_base_id == kb_id
    }
    
    clusters = {
        cluster_id: cluster
        for cluster_id, cluster in dependencies.clusters.items()
        if cluster.knowledge_base_id == kb_id
    }
    ...
    return BuildSuggestionList(
        suggestions=[BuildSuggestion.from_orm(s) for s in db_suggestions],
...
async def list_build_suggestions(
    ...
    return BuildSuggestionList(
        suggestions=[BuildSuggestion.from_orm(s) for s in suggestions],
        total=len(suggestions)
    )


@router.patch("/{kb_id}/suggestions/{suggestion_id}", response_model=BuildSuggestion)
async def update_build_suggestion(
    ...
    return BuildSuggestion.from_orm(suggestion)
